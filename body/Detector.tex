\section{ALICE Detector and data sample}
\label{Sec:Detector}
...
Out side of the TPC and located at a radius of approximately 4~m is the TOF, which measures the time-of-flight of the particles. The TOF is a cylindrical array of multi-gap resistive plate chambers with an intrinsic time resolution of 50~ps. It covers the pseudo-rapidity range $|\eta| < 0.9$ with (almost) full azimuthal acceptance. It can provide particle identification over a broad range at intermediate transverse momenta ($0.5 \lesssim \pT \lesssim 2.7$~\GeVc). The total time-of-flight resolution, including the resolution on the collision time, is about 90~ps in pp collisions(\com{some paper write as 100~ps. and how many for \pPb?}).
